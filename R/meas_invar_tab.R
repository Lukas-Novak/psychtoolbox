...
meas.invar.tab = function(RMSEA.dif) {
  if(!is.logical(RMSEA.dif)) stop("RMSEA.dif argument must be logical i.e. TRUE of FALSE")

# v těle funkce se bude zvyšovat počet sloupců dle zvolených parametrů viz RMSEA.dif
  # v těle funkce taky bude muset být obsažena knihovna semTools aby bylo možné rovnou odhadnout model

  if(RMSEA.dif == TRUE) tibble("RMSEA.dif" = ) # odsud dál pokračovat již příkazem, který máme v IRI
}
